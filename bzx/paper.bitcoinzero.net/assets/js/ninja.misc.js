ninja.seeder = {
    init: (function () {
        if (document.getElementById("generatekeyinput")) {
    ...
    showPoint: function (x, y) {
        ...
        var progression = Math.ceil(percentageComplete * 100);
        console.log(progression);
        document.getElementById("progress-bar-percentage").style.width = progression + "%";
        document.getElementById('progress-value').innerHTML = progression; // added progress-value
...
// edited to support responsive-tabbed-navigation
ninja.tabSwitch = function (walletTab) {
    console.log(walletTab);
    if (walletTab.className.indexOf("cd-selected") == -1) {
        console.log(ninja.wallets);
        // unselect all tabs
        for (var wType in ninja.wallets) {
            document.getElementById(wType).className = "tab";
            ninja.wallets[wType].close();
        }
        walletTab.className += " cd-selected";
...
ninja.envSecurityCheck = function () {
    ...
    if (document.getElementById("envSecurityCheck")) {
...
ninja.browserSecurityCheck = function () {
    ...
    if (document.getElementById("browserSecurityCheck")) {
...
ninja.runSerialized = function (functions, onComplete) {
    ...
    if (functions.length === 0) onComplete();
        ...
        f(function () {
            ninja.runSerialized(functions, onComplete);
        });
    }
};
...
ninja.forSerialized = function (initial, max, whatToDo, onComplete) {
    ...
    if (initial === max) {
        ...
        whatToDo(initial, function () {
            ninja.forSerialized(++initial, max, whatToDo, onComplete);
        });
    }
};
